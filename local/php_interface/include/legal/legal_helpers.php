...
{
    return '<a href="' . legal_h($path) . '" target="_blank" rel="noopener">' . legal_var($host . $path) . '</a>';
}
...
{
    $href = legal_doc_href($legal, $key);

    return legal_link($href, $text);
}
...
function oftalMedLegalFormPersonalDataText(): string
{
    static $text = null;
    if ($text !== null) {
        return $text;
    ...
    $text = 'При отправке формы Вы даёте '
        . '<a href="' . legal_h($legal['images']['consent']) . '" target="_blank">согласие на обработку персональных данных</a> и принимаете '
        . '<a href="' . legal_h($legal['images']['personal_data']) . '" target="_blank">политику обработки персональных данных</a>.';
