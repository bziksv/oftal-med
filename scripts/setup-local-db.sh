...
# Homebrew mysql 9 client breaks mysql_native_password; prefer 8.0
...
if [ "$TABLES" -ge 50 ] && [ "$FORCE_IMPORT" = false ]; then
  ...
  if [ "$BG" = true ]; then
    ...
    " >> "$LOG_FILE" 2>&1 &
    echo $! > "$PID_FILE"
    echo "Import started in background (pid $(cat "$PID_FILE"), nice 19)."
    echo "Progress: tail -f $LOG_FILE"
    # apply config immediately so site can boot while import runs
  ...
  case "$SQL_PATH" in
    *.gz)
      nice -n 19 gunzip -c "$SQL_PATH" | "$MYSQL" --protocol=SOCKET -u root \
        --default-character-set=utf8mb4 --max_allowed_packet=64M "$DB_NAME"
      ;;
    *)
      nice -n 19 "$MYSQL" --protocol=SOCKET -u root \
        --default-character-set=utf8mb4 --max_allowed_packet=64M \
        "$DB_NAME" < "$SQL_PATH"
      ;;
  esac
