...
if "$MYSQLADMIN" --protocol=SOCKET ping --silent 2>/dev/null; then
  ...
  TABLES=$("$MYSQL" -h 127.0.0.1 -u oftal_med_local -poftal_med_local -N -e \
    "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='oftal_med_ru_db'" 2>/dev/null || echo 0)
  if [ "$TABLES" -lt 50 ]; then
    ...
    echo "Local MySQL: oftal_med_ru_db ($TABLES tables)"
  fi
else
...
HTTP=$(curl -sS -o /tmp/oftal-med-check.html -w '%{http_code}' --max-time 60 http://127.0.0.1:8101/ || echo 000)

echo "soft http://127.0.0.1:8101/ → HTTP $HTTP"
echo "php-fpm :9101 (ondemand, max 2 workers, 512M)"
