<?
namespace Arturgolubev\Chatgpt; //2.3.2
	...
	static function checkModuleDemoEx($module_id){
		...
		if($rq['checkperiod'] == 'Y'){
			...
			if(true){
				...
				if($obCache->InitCache(600, 'ag_module_getdata'.$module_id, '/check_mp/ag_module_getdata')){
			...
			$showMessage = Option::set($module_id, 'license_message', $message);
		...
		if($showMessage){
			$html.= '
				<div class="module-license-info">
					<div class="module-license-info-icon"></div>
					<div class="module-license-info-text">
						'.$showMessage.'
					</div>
				</div>
			';
			...
			BX.ready(function(){
				...
				BX.ajax({
					...
					onsuccess: function(data){
						// console.log("GetLicense SUCCESS");
					},
					onfailure: function(){
