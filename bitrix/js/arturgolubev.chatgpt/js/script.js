...
agcg.initAppendFormActions = function (){
	...
	if(savedTemplate.length){
		savedTemplate.forEach(function(item){
			item.addEventListener('change', function(){
				var t = this, textarea = t.closest('table').querySelector('textarea');
				if(t.value != '0'){
	...
	BX.ajax({
		...
		onsuccess: function(data){
			...
			if(data.error){
				...
				}else if(data.renew_request && options.renew < 2){
					result.innerHTML = result.innerHTML + '<br>' + data.error_message + '. ' + BX.message("ARTURGOLUBEV_CHATGPT_JS_RENEW_REQUEST");

					options.first = 0;
					options.renew = options.renew + 1;
					agcg.requestImagePage(_this, options);
				}else{
					result.innerHTML = result.innerHTML + '<br>' + data.error_message;
				}
			}else{
				let newHtml = '<img src="'+data.image+'" alt="" /><br><br><a target="_blank" href="'+data.image+'">'+BX.message("ARTURGOLUBEV_CHATGPT_JS_IMAGE_IN_NEWTAB")+'</a>';
...
agcg.massWindowShow = function(params, elements){
	...
	if(params.demo){
		wContent = wContent + '<div style="font-size: 13px; color: #444;">'+BX.message('ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_DEMO_NOTIFY')+' ' + params.dcount + '. ' + BX.message('ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_DEMO_NOTIFY_CONTINUE') +'</div><br/>';
	}
	
	wContent = wContent + '<div style="font-size: 13px; color: #444;">'+BX.message('ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_BACKUP_NOTIFY')+'</div><br/>';
	
	wContent = wContent + '<div style="font-size: 13px; color: #444;">';
		if(params.entity == 'elements'){
			wContent = wContent + BX.message('ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_SELECTED_ELEMENTS');
		}else{
			wContent = wContent + BX.message('ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_SELECTED_SECTIONS');
		}
		
		
		wContent = wContent + ' (' +elements.length+ ') :';
				...
				BX.ajax({
					...
					onsuccess: function(data){
						...
						if(data.used_tokens_cnt){
							params.token_price = data.token_price;
						...
						if(params.nodes.provider.length){
							...
							if(provider == 'chatgpt'){
								if(!data.gpt_model){
									params.skip_sleep = 1;
								}else if(data.gpt_model.indexOf('3.5') < 0){
									params.skip_sleep = 1;
								}
							}
						}
	...
	agcg.massGenerateEnd = function(params){
		...
		if(params.show_tokens && params.used_tokens_cnt){
			agcg.logAddLine(params.nodes.results, '<br>' + BX.message('ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_USED_TOKENS') + params.used_tokens_cnt + '. ' + BX.message("ARTURGOLUBEV_CHATGPT_JS_MASS_WINDOW_USED_TOKENS_PRICE") + (params.used_tokens_cnt * params.token_price / 1000));
		}
			...
			if(preview_mode){
				...
				if(data.error){
					...
					if(data.content_type == 'image'){
						html = html + '<div class="result-textareas"><div class="label">' + BX.message("ARTURGOLUBEV_CHATGPT_JS_ELEMENT_GENERATE_SUCCESS_RESULT") + '</div><input type="hidden" name="genresult" value="' + data.answer + '"><img src="' + data.answer + '" alt="" /></div>';
					}else{
